...
.libPaths(c(bibliotheque,.libPaths()))

bibliotheque <- "../Package install 3.3/Test1/.checkpoint/2018-01-01/lib/x86_64-w64-mingw32/3.3.3/"

# library(checkpoint,lib.loc = bibliotheque)
library(sp,lib.loc = bibliotheque)
library(rgdal,lib.loc = bibliotheque)
library(rgeos,lib.loc = bibliotheque) 
library(leaflet,lib.loc = bibliotheque)
library(leaflet.extras,lib.loc = bibliotheque) # 
library(rlang,lib.loc = bibliotheque) #
# library(lattice,lib.loc = bibliotheque)
library(Formula,lib.loc = bibliotheque)
library(ggplot2,lib.loc = bibliotheque)
library(backports,lib.loc = bibliotheque)
library(Hmisc,lib.loc = bibliotheque)
library(crayon,lib.loc = bibliotheque)
library(cli,lib.loc = bibliotheque)
library(tibble,lib.loc = bibliotheque)
library(tidyr,lib.loc = bibliotheque)
library(readr,lib.loc = bibliotheque)
library(purrr,lib.loc = bibliotheque)
library(dplyr,lib.loc = bibliotheque)
library(stringr,lib.loc = bibliotheque)
library(forcats,lib.loc = bibliotheque)
library(tidyverse,lib.loc = bibliotheque)
library(fst,lib.loc = bibliotheque)
library(fstplyr)
library(shiny,lib.loc = bibliotheque) # Graphic web interface
library(shinyBS,lib.loc = bibliotheque) # Pop-up windows
library(shinyjs,lib.loc = bibliotheque) # Mask buttons/elements on graphic interface
library(shinyWidgets,lib.loc = bibliotheque) # Widgets supplementaires
library(shinydashboard,lib.loc = bibliotheque) # Tools like infoBox
library(shinycssloaders,lib.loc = bibliotheque) # spinner loader
library(RColorBrewer,lib.loc = bibliotheque) 
library(plotly,lib.loc = bibliotheque)
library(DT,lib.loc = bibliotheque)
library(Rcpp,lib.loc = bibliotheque)
# library(slam,lib.loc = bibliotheque)
# library(Rglpk,lib.loc = bibliotheque)
# library(sdcTable,lib.loc = bibliotheque)
# library(easySdcTable,lib.loc = bibliotheque)
library(rintrojs,lib.loc = bibliotheque) # Tutoriel intéractif
library(knitr,lib.loc = bibliotheque)
library(yaml,lib.loc = bibliotheque)
library(markdown,lib.loc = bibliotheque)
library(bindrcpp,lib.loc = bibliotheque)
library(tidyselect,lib.loc = bibliotheque)
library(labeling,lib.loc = bibliotheque)
library(utf8,lib.loc = bibliotheque)


# Fonctions  particulières
source("Other programs/2 Fonctions Agate/4 Dashboard/Agate - Dashboard fct.R",encoding = "UTF-8")
source("Other programs/2 Fonctions Agate/3 Indicateurs statistiques/Agate - Statistics Zonage_v11.R",encoding = "UTF-8")
...
server = function(input, output, session) {
  ...
  observeEvent(rv$dash.indicateur,{
    ...
    if(!is.null(rv$dash.indicateur) & !is.null(rv$zone.comparaison)){
      ...
      output$dt_dem_bd = renderDataTable(
      ...
      output$dt_sco_hg = renderDataTable(
        datatable(rv$dash.indicateur$df.sco.tab.hg, colnames = rv$dash.indicateur$dash.label,
                  rownames = FALSE, options = list(dom = 't'))
      )

      output$g_sco_hd = renderPlotly({
        hide_legend(rv$dash.indicateur$g.sco.pop)
      })

      output$g_sco_bg = renderPlotly({
        hide_legend(ggplotly(rv$dash.indicateur$g.sco.diplome))
      })

      output$dt_sco_bd = renderDataTable(
      ...
      output$dt_res_hg = renderDataTable(
        datatable(rv$dash.indicateur$df.res.tab.hg, colnames = rv$dash.indicateur$dash.label,
                  rownames = FALSE, options = list(dom = 't'))
      )

      output$g_res_hd = renderPlotly({
        hide_legend(rv$dash.indicateur$g.res.nbp)
      })

      output$g_res_bg = renderPlotly({
        hide_legend(rv$dash.indicateur$g.res.surf)
      })

      output$dt_res_bd = renderDataTable(
  ...
  observeEvent(c(input$si_stat_categorie,input$si_stat_domaine,input$pi_stat_zone_etude),{
    ...
    if(input$si_stat_categorie != ""){
      ...
      if(!is.null(df)){
        ...
        # Affichage
        output$dt_stat_explore = renderDataTable(
          datatable(df,
                    colnames = type.ind,
                    extensions = 'Buttons',
                    options = list(
