...
ril <- read_fst(rilPath) %>%
  rename(nb_logn = NB_LOG,
         x=X,
         y=Y) %>% 
  mutate(C_ANNEE_COL = as.character(millesime)) %>% 
  select(idx,C_ANNEE_COL,nb_logn,x,y) %>% 
...
ril.unique <- ril[duplicated(ril@data$idx)==F,]
pts.sp <- zonaPts(pts.sp = ril.unique,zonage = zonage)
pts.df <- left_join(pts.sp@data %>% select(idx,idZonage),
                    ril@data %>% select(idx,C_ANNEE_COL,nb_logn),
                    by="idx")
...
rpi <- read_fst(rpiPath) %>% 
  filter(idx %in% unique(ril@data$idx)) %>% 
  left_join(pts.df, by = c("idx","C_ANNEE_COL")) %>% 
...
# VI. Qualité des données du RP (Travaux Baptiste Raimbaud)
#----------------------------------------------------------
incProgress(amount = 0.5,message = "Qualité des données du rp")

# VI.1. Chargement de la base adresses (MAJ : 19.03.2019)
...
# RIL : A revoir avec Baptiste. Problème lié au RIL millesimé. Il faut choisir comment calculer le nombre de logements
# Pour l'instant, je choisis de ne retenir que les logements du ril concerné sur les 5 années de collecte
rilqualite <- pts.df %>% 
  filter(substr(C_ANNEE_COL,3,4) %in% as.character(seq(as.numeric(rp.an)-2,as.numeric(rp.an)+2,1))) %>% 
  mutate(idZonage = ifelse(is.na(idZonage) | idZonage == "Hors zonage","horsZon", idZonage),
         com = substr(idx,1,5)) %>%
  mutate(idZonage = ifelse(idZonage=="horsZon",paste0(idZonage,com),idZonage))
...
# X. Enregistrement temporaire pour test
#---------------------------------------------------------------------------------------------------------------------------------------------
