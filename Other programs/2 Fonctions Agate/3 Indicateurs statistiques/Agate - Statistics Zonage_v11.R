...
agate_statRp <- function(rp.an,zonage,zone.pred,zoneType = "",group_var,com.dom,rpi.weight,rpl.weight,
                         ...
                         CRSagate = "+init=EPSG:3857"){ # MAJ 29.12.2019
  ...
  zonage <- spTransform(zonage, CRSagate)
  com.dom <- spTransform(com.dom, CRSagate)
  ...
  rpa.geo@proj4string <- CRS(CRSagate)
  ...
  pts.sp <- zonaPts(pts.sp = rpa.geo,zonage = zonage,CRSagate = CRSagate)
  pts.df <- pts.sp@data %>% 
...
# Indicateurs statistiques à partir de variables qualitatives (MAJ : 29.12.2019)
agate_qualitative <- function(indicateur,df,group_var,poids){
  df %>% 
    rename(nomIndicateur = !!!syms(indicateur)) %>% 
