##############################################################################################################
#################### Nicolas Kempf - Fonctions Cartographiques ###############################################
##############################################################################################################
...
# RIL
#   Repertoire d'immeubles localisés
#   Obligatoirement un objet de type SpatialPointDataFrame
#   Variables indispensables
#     id : construit de la manière suivante : Codecommune + 3 espaces + identifiant Ilot + 1 espace + numéro de rang
...
#
#######################################################
zonaRil <- function(ril,zonage){

  #### Changement du système de projection : Utilisation de epsg:3857 
  ril <- spTransform(x = ril,CRSobj = CRS("+init=epsg:3857"))
  ...
  ######################################
    ...
    zonageDB <- over(ril,zonage)
    # Ajout des identifiants des points
    zonageDB <- data.frame(idx=ril@data$idx,zonageDB)
    
    # Ajout des variables d'interets de zonage au RIL
    ril <- merge(ril,zonageDB,by="idx",all.x=T)
